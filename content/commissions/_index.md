...
<style>
    /* 1. LAYOUT & THEME */
    html, body {
        background-color: #0b0510 !important; /* Deep midnight */
        background-image: 
    ...
    /* 2. DYNAMIC CANVAS BACKGROUND BACKGROUND (Falling art & sparkle trails) */
    #art-rain-canvas {
        position: fixed;
        top: 0;
        left: 0;
...
<script>
    // --- LIGHTWEIGHT ART RAIN & SPARKLE TRAIL ENGINE ---
    const canvas = document.getElementById('art-rain-canvas');
    const ctx = canvas.getContext('2d');

    // List of your commission image endpoints to float down
    const artSources = [
    ...
    // Preload your illustrations safely
    ...
    class FallingArt {
        constructor() {
            ...
            this.size = rand(45, 75); // Small icon sizing
            this.x = rand(-this.size, canvas.width);
            this.y = rand(-this.size * 2, -this.size);
            this.speedY = rand(0.6, 1.4); // Sweet, slow downward drifting speed
        ...
        update() {
            ...
            // Drop a trail sparkle behind each card every few frames
        ...
        draw() {
            ...
            // Soft drop shadow to float over the radial space background cleanly
            ctx.shadowColor = 'rgba(157, 80, 187, 0.3)';
            ctx.shadowBlur = 8;

            // Clip into a rounded square look matching the site UI cards
            ctx.beginPath();
    ...
    class TrailSparkle {
        constructor(x, y) {
            ...
            // Cute palette matching the site accents
            const colors = ['#9d50bb', '#2fb98f', '#ffffff', '#e1bee7', '#80deea'];
            this.color = colors[Math.floor(Math.random() * colors.length)];
            this.speedY = rand(0.1, 0.4); // Drifts down extra slowly as it fades
        }
        ...
        draw() {
            ...
            // Draw a cute star-like diamond sparkle shape
            ctx.beginPath();
